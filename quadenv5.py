...
class QuadEnv_polyv3:
    def __init__(self,num_envs:int,show_viewer:bool=False):
        ...
        #1.5、增加目标
        self.target=self.scene.add_entity(morph=gs.morphs.MJCF(file="xml/car.xml",scale=0.5))
        #1.6、增加无人机
        self.drone=self.scene.add_entity(gs.morphs.Drone(file="urdf/drones/cf2x.urdf"))
        #1.7、添加视角
        self.cam = self.scene.add_camera(res=(640, 480),pos=(3.5, 0.0, 2.5),lookat=(0, 0, 0.5),fov=30,GUI=True,)#用于展示
        #1.8、build scene
        self.scene.build(n_envs=self.num_envs)
        #2、初始化环境变量
        #2.1、无人机初始位置
        self.base_init_pos=torch.tensor([0.0, 0.0, 1.0],device=gs.device)#从地面高度1.5m出发
        ...
        #2.2、环境状态变量
        self.num_obs=17#位置（3）、线速度（3）、角度四元数（4）、角速度（3）、动作（4）
        self.num_actions=4
        self.num_commands=3#位置（3）
        self.num_privileged_obs = None
        #2.3奖励设置
        self.reward_scales={"target": 10.0,"smooth": -1e-3,"crash": -10.0,}
        ...
        #2.5、环境缓冲区
        self.obs_buf = torch.zeros((self.num_envs, self.num_obs), device=gs.device, dtype=gs.tc_float)#观测值缓冲区
        self.rew_buf = torch.zeros((self.num_envs,), device=gs.device, dtype=gs.tc_float)#奖励缓冲区
        self.reset_buf = torch.ones((self.num_envs,), device=gs.device, dtype=gs.tc_int)#重置缓冲区，初始全1保证开始环境初始化
        self.episode_length_buf = torch.zeros((self.num_envs,), device=gs.device, dtype=gs.tc_int)#每个环境的当前episode长度
        self.target_pos_buf = torch.zeros((self.num_envs, self.max_episode_length,self.num_commands), device=gs.device, dtype=gs.tc_float)#目标位置缓冲区
        #self.target_pos=torch.zeros((self.num_envs, self.num_commands), device=gs.device, dtype=gs.tc_float)#当前目标位置
        self.actions = torch.zeros((self.num_envs, self.num_actions), device=gs.device, dtype=gs.tc_float)#动作缓冲区
    ...
    def target_move_idx(self,env_idx):
        ...
        name='/root/quad_rl/mobile_data_poly/'+str(num)+'.npy'
        data=np.load(name)
        data=data.reshape(-1,2)
        z=np.full((data.shape[0],1),0.20)#小车几何中心0.15，小车表面厚度0.05，因此无人机目标位置高度为0.2
        data=np.concatenate((data,z),axis=1)
        self.target_pos_buf[env_idx] = torch.as_tensor(data, device=gs.device,dtype=gs.tc_float)
    ...
    def reset_idx(self,env_idx):
        ...
        self.drone.set_quat(self.base_quat[env_idx], zero_velocity=True, envs_idx=env_idx)
        ...
        for key in self.episode_sums.keys():
            self.extras["episode"]["rew_" + key] = (torch.mean(self.episode_sums[key][env_idx]).item() / self.episode_length)
    def reset(self):
    ...
    def at_target(self):
        ...
        cond = (torch.norm(self.rel_pos[:, :2], dim=1) < 0.1) & (torch.abs(self.rel_pos[:, 2]) < 0.03)
    ...
    def step(self,actions):
        ...
        target_pos = self.target_pos_buf[torch.arange(self.target_pos_buf.shape[0]), self.episode_length_buf%self.max_episode_length, :]
        ...
        self.base_euler =geom.quat_to_xyz(geom.transform_quat_by_quat(torch.ones_like(self.base_quat) * self.inv_base_init_quat,self.base_quat),rpy=True,degrees=True)
        inv_base_quat = geom.inv_quat(self.base_quat)
        self.base_lin_vel[:] = geom.transform_by_quat(self.drone.get_vel(), inv_base_quat)
        self.base_ang_vel[:] = geom.transform_by_quat(self.drone.get_ang(), inv_base_quat)
        ...
        self.crash_condition = (
            ...
            | (self.base_pos[:, 2] < 0.1))
        self.reset_buf=(self.episode_length_buf > self.max_episode_length) | self.crash_condition | self.at_target()
